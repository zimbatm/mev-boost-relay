...
func TestRedisValidatorRegistration(t *testing.T) {
	...
	t.Run("Can save and get validator registration from cache", func(t *testing.T) {
		...
		cache.SetValidatorRegistration(ctx, value)
		result, err := cache.GetValidatorRegistration(ctx, key.PubkeyHex())
		require.NoError(t, err)
...
func TestRedisKnownValidators(t *testing.T) {
	...
	t.Run("Can save and get known validators", func(t *testing.T) {
		...
		require.NoError(t, cache.SetKnownValidator(ctx, key1, 1))
		require.NoError(t, cache.SetKnownValidator(ctx, key2, 2))

		knownVals, err := cache.GetKnownValidators(ctx)
		require.NoError(t, err)
...
func TestRedisValidatorRegistrations(t *testing.T) {
	...
	t.Run("Can save and get validator registrations", func(t *testing.T) {
		...
		numReg, err := cache.NumRegisteredValidators(ctx)
		require.NoError(t, err)
		...
		cache.SetValidatorRegistration(ctx, entry)

		numReg, err = cache.NumRegisteredValidators(ctx)
		require.NoError(t, err)
		require.Equal(t, int64(1), numReg)

		reg, err := cache.GetValidatorRegistration(ctx, key1)
		require.NoError(t, err)
